...
execute in battlepaint:battlepaint run tellraw @a[tag=battlepaint] {"text":"玩家 [绿色] ","color":"dark_green","extra":[{"selector":"@s"},{"text":" 加入了油漆大战","color":"gold"}]}
tellraw @s {"text":"你已成功加入绿色队伍","color":"dark_green","bold":true}
tellraw @s {"text":"请等待全部玩家到齐后，输入 /trigger battlepaint_start 开启游戏即可，务必确保所有玩家都已选择颜色","color":"yellow","bold":true}
tellraw @s {"text":"如果想在游戏开始前退出游戏或重新选择颜色，请输入 /trigger battlepaint_tplobby 来回到大厅","color":"yellow","bold":true}

fill ~-1 7 ~-1 ~1 7 ~1 minecraft:green_concrete
